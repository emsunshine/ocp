...
def radius_graph_pbc(
    ...
    # Calculate the number of required repeats in each direction
    repeats = radius / torch.linalg.norm(data.cell, ord=2, dim=-1)
    
    # Round up to whole numbers
    repeats = torch.ceil(repeats)
    
    # Take the max across all structures in the batch 
    # Opportunity for speedup in the future
    max_rep = torch.max(repeats, dim = 0).values
    
    # Mask out dimensions where pbc was disabled
    max_rep[~torch.tensor(pbc)] *= 0
